...
for(y in 1:length(yr)){
  ...
  tmp_KMT = as.data.frame(tmp_data_bytown[,c("countyname","townname","KMT_vote")])
  ...
  tmp_DPP = tmp_data_bytown[,c("countyname","townname","DPP_vote")]
  tmp_DPP = merge(taiwan_shp,tmp_DPP,by = c("countyname","townname"))

  
  
  test = tmp_DPP
  test$diff = tmp_DPP$DPP_vote - tmp_KMT$KMT_vote
  test$diff[which(test$diff> 10000)] = 10000
  test$diff[which(test$diff< -10000)] = -10000
  plot_title = sprintf("%s 不分區藍綠版圖",year)
  p3 = ggplot(test,
              aes(fill = diff)) +
    geom_sf() + 
    scale_fill_gradient2(low = "blue",high = "green",mid = "white",midpoint = 0,limits=c(-10000,10000)) +
    coord_sf(xlim= c(119,123)) + 
    ggtitle(plot_title) +
    guides(fill=guide_legend(title="藍綠版圖"))
  outfile = sprintf("%s/plots/%s_DDP_KMT_不分區_人數.png",path,year)
